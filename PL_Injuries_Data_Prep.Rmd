...
hamstring_only <- hamstring_only %>%
                    ...
                    select(-Position_Goalkeeper, -Foot_Right) #these are the "base" values
```
...
## FINAL TESTING TO BE DONE ON LAST YEAR's DATA
```
...
test_truth <- data_test %>% select(injured)
```
...
```{r}
fun_calc_auc(glm_prediction_probs, test_truth)
```
